...
class RecipeTests(TestCase):
    ...
    def test_recipe_decompose_ingredients(self):
        with self.subTest(""):
            ingredients = Recipe.objects.filter(name='Traditional Filipino Lumpia').first().decompose_recipe()
            for ingredient in ingredients:
                print(ingredient)

            self.assertEqual("meow", "meow")

    def test_recipe_xx(self):
        pass