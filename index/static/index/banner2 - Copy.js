...
class Banner {
    ...
    addBranch(x, y, angle, length, depth, isActive) {
        ...
        if (depth === 0) {
            let childrenCount = Math.floor(GetRandomBetween(1, 7));
    ...
    drawBranches(delta) {
        ...
        for (let a = 0; a < this.branchTypes.length; a++) {
            ...
            for (let i = 0; i < this.branchPool.length; i++) {
                ...
                growSpeed *= Banner.getInstance().trees;
                ...
                let totalGrown = branch.length - branch.lengthLeft - growLength;

                let endPoint = branch.getEndPoint(growLength);
                if (endPoint.y < 50) endPoint.y = 80;
                ...
                while (branch.children && totalGrown > branch.children.peek()) {
                    ...
                    let ci = this.addBranch(branch.x, branch.y, angle, length, depth, false);
                ...
                if (finalGrow) {
                    if (branch.depth < 3 && !branch.isNeedle) {
                        let needleCount = 0;
                        if (branch.depth === 0) needleCount = 10;
                        else if (branch.depth === 1) needleCount = 1;
                        else if (branch.depth === 2) needleCount = 1;
                        else needleCount = 1;
   ...
   static render(currentTime) {
        ...
        if (banner.isFirstFrame) {
            ...
            banner.trees = 0;
            banner.treeBuckets = [];
            banner.treeBucketCount = Math.floor(GetRandomBetween(4, 6));
            for (let i = 0; i < banner.treeBucketCount; i++) {
                banner.treeBuckets.push(GetRandomBetween(0, banner.width));
            }
            ...
            banner.timer.createRecurringTimer(500, function(time) {
                if (banner.stopTrees) return;
                banner.trees++;
                let search = -1;
                let color;
                if (banner.trees > 30) search = 1;
                ...
                if (banner.trees > 1) return;
                let bucketId = Math.floor(GetRandomBetween(0, banner.treeBucketCount));
                banner.drawTree(banner.treeBuckets[bucketId] + GetRandomBetween(0, 20), 0, GetRandomBetween(80, 100), GetRandomBetween(15, 22), color);
            });
        } else {
            banner.frames++;
